...
public class LockOpenCardAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> implements View.OnClickListener {
    private Context mContext;
    private List<LockKeysBean> list;
//    private static final int TYPE_COMMON = 1;
//    private static final int TYPE_TEMPORARY = 2;
    ...
    public LockOpenCardAdapter(Context context, List<LockKeysBean> list) {
        mContext = context;
        this.list = list;
    }
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        //创建viewholder
        if (viewType == USER_TYPE_COMMON) {
            View view = LayoutInflater.from(mContext).inflate(R.layout.lock_keys_owner_item, parent,
                    false);
            view.setOnClickListener(this);
            return new OwerItemViewHolder(view);
        } else if (viewType == USER_TYPE_TEPORARY) {
            View view = LayoutInflater.from(mContext).inflate(R.layout.lock_keys_temporary_item, parent,
                    false);
            view.setOnClickListener(this);
    ...
    public int getItemViewType(int position) {
        ...
        if (userType == USER_TYPE_TEPORARY) {
            return USER_TYPE_TEPORARY;
        } else {
            return USER_TYPE_COMMON;
        }
    }
